...
class Provider(models.Model):
    ...
    class Meta:
        ordering = ["name"]
        constraints = [
            models.UniqueConstraint(Lower("name"), name="uq_provider_name_ci"),
        ]
...
class Bill(models.Model):
    ...
    total = models.DecimalField(max_digits=14, decimal_places=3, default=0, validators=[MinValueValidator(0)])
    ...
    def assign_serial_if_needed(self):
        if self.serial:
            return
        # simple max+1 with retry (like products)
        for _ in range(5):
            try:
                with transaction.atomic():
                    last = Bill.objects.select_for_update().order_by("-serial").values_list("serial", flat=True).first()
...
class BillItem(models.Model):
    ...
    qty_primary = models.DecimalField(max_digits=14, decimal_places=3)   # stored in primary units
    cost = models.DecimalField(max_digits=12, decimal_places=4, validators=[MinValueValidator(0)])   # cost per primary unit
    price = models.DecimalField(max_digits=12, decimal_places=4, validators=[MinValueValidator(0)])  # price per primary unit at time of bill
