...
def finalize_pos_bill(*, bill: SalesBill, actor) -> None:
    ...
    for row in rows:
        ...
        InvSV.record_sale_item(
            ...
            unit_1_label_at_txn=getattr(row, "unit_1_label_at_txn", "") or "",
            unit_2_label_at_txn=getattr(row, "unit_2_label_at_txn", "") or "",
            product_name_at_txn=getattr(row, "product_name_at_txn", None) or getattr(row, "product_name", None) or "",
            source_app="pos",
            source_model="SalesBill",
            source_id=str(bill.id),
            container=container,
