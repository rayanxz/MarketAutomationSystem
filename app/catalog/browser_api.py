...
def api_browser_collections(request):
    try:
        ...
        page = 1
    qs = ProductCollection.objects.order_by(Lower("name")).values("id", "name", "code")
...
def api_browser_sets(request):
    ...
    qs = ProductSet.objects.filter(collection_id=cid).order_by(Lower("name")).values("id", "name", "code")
    total, pages, page, off, size = _page(qs, page)
    return JsonResponse({
        "ok": True,
        "level": "sets",
...
def api_browser_products(request):
    ...
    if not sid:
        return JsonResponse({"ok": True, "items": [], "total": 0, "page": 1, "total_pages": 1})
    qs = (Product.objects.filter(set_id=sid)
          .order_by("product_number")
          .values("id", "name", "product_number"))
    total, pages, page, off, size = _page(qs, page)
    items = [{"id": p["id"], "name": p["name"], "code": f'{p["product_number"]:03d}'} for p in qs[off:off+size]]
    return JsonResponse({
        "ok": True,
        "level": "products",
        "items": items,
