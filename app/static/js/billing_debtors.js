...
(function () {
  ...
  mFullConfirm.addEventListener("click", async () => {
    if (!target.id) return;
    try{
      const resp = await fetch(API.PAY_FULL(target.id), { method: "POST", headers: { "X-CSRFToken": getCsrf(), "Accept":"application/json" } });
  ...
  mBatchConfirm.addEventListener("click", async () => {
    ...
    try{
      const resp = await fetch(API.PAY_BATCH(target.id), { method: "POST", body: form, headers: { "X-CSRFToken": getCsrf(), "Accept":"application/json" } });
      const data = await resp.json();
